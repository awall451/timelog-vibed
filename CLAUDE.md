...
- **`frontend/src/lib/demo/db.ts`** — `sql.js` init via `initSqlJs({ locateFile: () => sqlWasmUrl })`, fetches `/seed/timelog.db` on first load, hydrates from IndexedDB key `timelog-demo-db-v2` (bump the version when the seed changes materially; legacy keys are deleted on load) on subsequent loads, persists after every mutation, exposes `reset()`.
