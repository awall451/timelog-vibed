from __future__ import annotations

import re
from datetime import datetime
from pathlib import Path

import click
from tabulate import tabulate

from timelog import service
from timelog.ai_sync import (
    ALL_SOURCES,
    ProposedEntry,
    active_hours,
    build_proposed_entries,
    build_proposed_entries_with_ai,
    check_duplicates,
    claude_source,
    cursor_source,
)


def _print_entries_table(entries: list[ProposedEntry]) -> None:
    rows = [
        [e.project, "+".join(e.sources), e.category, e.hours, e.description]
        for e in entries
    ]
    click.echo(tabulate(
        rows,
        headers=["Project", "Source", "Category", "Hours", "Description"],
        tablefmt="psql",
    ))


def _resolve_date(date: str | None) -> str:
    if date is None:
        return datetime.now().strftime("%Y-%m-%d")
    if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", date):
        raise click.ClickException(f"Date must be in YYYY-MM-DD format, got: {date!r}")
    return date


def _resolve_sources(sources: tuple[str, ...]) -> list[str]:
    if not sources or "all" in sources:
        return list(ALL_SOURCES)
    return list(sources)


def _source_option(fn):
    return click.option(
        "--source",
        "sources",
        multiple=True,
        type=click.Choice(["claude", "cursor", "all"]),
        default=("all",),
        help="AI sources to pull from (default: all). Repeatable.",
    )(fn)


@click.group("tlclaude", invoke_without_command=True)
@click.pass_context
def claude(ctx: click.Context) -> None:
    """Generate timelog entries from AI coding-assistant session data (Claude + Cursor)."""
    if ctx.invoked_subcommand is None:
        click.echo(ctx.get_help())


@claude.command("sessions")
@click.option("--date", default=None, metavar="YYYY-MM-DD", help="Date to inspect (default: today)")
@_source_option
def sessions_cmd(date: str | None, sources: tuple[str, ...]) -> None:
    """List raw AI sessions for a date."""
    date_str = _resolve_date(date)
    resolved_sources = _resolve_sources(sources)

    all_sessions = []
    if "claude" in resolved_sources:
        try:
            all_sessions.extend(claude_source.load_sessions(date_str))
        except FileNotFoundError as e:
            click.echo(f"warn: Claude history not found: {e}", err=True)
    if "cursor" in resolved_sources:
        try:
            all_sessions.extend(cursor_source.load_sessions(date_str))
        except FileNotFoundError as e:
            click.echo(f"warn: Cursor history not found: {e}", err=True)

    if not all_sessions:
        click.echo(f"No AI sessions found for {date_str}.")
        return

    rows = []
    for s in sorted(all_sessions, key=lambda s: min(s.timestamps) if s.timestamps else datetime.min):
        project_name = Path(s.project_path).name
        start = min(s.timestamps).strftime("%H:%M") if s.timestamps else "—"
        hours = active_hours(s.timestamps)
        branches = ", ".join(s.git_branches) or "—"
        rows.append([s.source, project_name, start, f"{hours:.2f}h", branches, s.session_id[:8]])

    click.echo(f"\nDate: {date_str}  ({len(all_sessions)} session{'s' if len(all_sessions) != 1 else ''})\n")
    click.echo(tabulate(
        rows,
        headers=["Source", "Project", "Start", "Active", "Branch", "Session"],
        tablefmt="psql",
    ))


@claude.command("preview")
@click.option("--date", default=None, metavar="YYYY-MM-DD", help="Date to preview (default: today)")
@click.option("--no-ai", is_flag=True, default=False, help="Skip AI inference, use heuristics only")
@_source_option
def preview_cmd(date: str | None, no_ai: bool, sources: tuple[str, ...]) -> None:
    """Show proposed timelog entries without inserting."""
    date_str = _resolve_date(date)
    resolved_sources = _resolve_sources(sources)

    if not no_ai:
        click.echo("Analyzing sessions with Claude... ", nl=False)

    try:
        entries = (
            build_proposed_entries_with_ai(date_str, resolved_sources)
            if not no_ai
            else build_proposed_entries(date_str, resolved_sources)
        )
    except FileNotFoundError as e:
        raise click.ClickException(f"Source history not found: {e}")

    if not no_ai:
        click.echo("done.")

    if not entries:
        click.echo(f"No AI sessions found for {date_str}.")
        return

    click.echo(f"\nProposed entries for {date_str}:\n")
    _print_entries_table(entries)


@claude.command("sync")
@click.option("--date", default=None, metavar="YYYY-MM-DD", help="Date to sync (default: today)")
@click.option("--yes", "-y", is_flag=True, default=False, help="Skip confirmation prompt")
@click.option("--no-ai", is_flag=True, default=False, help="Skip AI inference, use heuristics only")
@_source_option
def sync_cmd(date: str | None, yes: bool, no_ai: bool, sources: tuple[str, ...]) -> None:
    """Generate and insert timelog entries from AI sessions."""
    date_str = _resolve_date(date)
    resolved_sources = _resolve_sources(sources)

    if not no_ai:
        click.echo("Analyzing sessions with Claude... ", nl=False)

    try:
        entries = (
            build_proposed_entries_with_ai(date_str, resolved_sources)
            if not no_ai
            else build_proposed_entries(date_str, resolved_sources)
        )
    except FileNotFoundError as e:
        raise click.ClickException(f"Source history not found: {e}")

    if not no_ai:
        click.echo("done.")

    if not entries:
        click.echo(f"No AI sessions found for {date_str}.")
        return

    new_entries, skipped = check_duplicates(entries)

    for e in skipped:
        click.echo(f"Skipping {e.project} — entry for {e.date} already exists.")

    if not new_entries:
        click.echo("All entries already synced.")
        return

    click.echo(f"\nProposed entries for {date_str}:\n")
    _print_entries_table(new_entries)
    click.echo()

    if not yes:
        click.confirm("Insert these entries?", abort=True)

    import sqlite3
    from timelog.db import DB_PATH
    try:
        for e in new_entries:
            service.add_entry(e.project, e.category, e.description, e.hours, e.date)
    except sqlite3.OperationalError as exc:
        if "readonly" in str(exc):
            raise click.ClickException(
                f"Database is read-only: {DB_PATH}\n"
                f"Fix: sudo chown $(whoami):$(whoami) {DB_PATH}"
            )
        raise

    n = len(new_entries)
    click.echo(f"Inserted {n} entr{'y' if n == 1 else 'ies'}.")
